#!/bin/bash
# MAGPIE baseline: Code Generation (few-shot) using Qwen2.5-Coder-7B-Instruct
# Requires a cloned MAGPIE repo: https://github.com/magpie-align/magpie
# Usage: bash run_codegen_fewshot.sh [model_path] [total_prompts]

MAGPIE_DIR="../magpie"  # Path to cloned MAGPIE repo
MODEL_PATH=${1:-"Qwen/Qwen2.5-Coder-7B-Instruct"}
TOTAL_PROMPTS=${2:-80000}
INS_TOP_P=1
INS_TEMP=0.8
RES_TOP_P=1
RES_TEMP=0
...
timestamp=${timestamp//-/}
job_name="code_gen_few_shot_${MODEL_PATH##*/}_topp${INS_TOP_P}_temp${INS_TEMP}_${timestamp}"
OUTPUT_DIR="./outputs/magpie/code-gen"
...
echo "[MAGPIE] Control Task: code_gen_few_shot"
...
CUDA_VISIBLE_DEVICES=$DEVICE python "${MAGPIE_DIR}/exp/gen_ins.py" \
    ...
    --job_name "$job_name" \
    --timestamp "$timestamp" \
    --control_tasks "code_gen_few_shot" \
